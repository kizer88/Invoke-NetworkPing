function Get-SCCMInventory {
    ...
    if (Test-Path $cachePath) {
        ...
        if ($cacheAge.TotalHours -lt 24) {
            ...
            if ($Global:CachedSystems -is [hashtable] -and
                $Global:CachedSystems.Count -gt 0 -and
                $Global:CachedSystems.Values | Where-Object { $_.DNSSuffix -and $_.IPAddresses }) {

                $global:DNSValidatorHash = @{}
                $global:DNSValidatorHash.allSystems = $Global:CachedSystems.Clone()
                Write-Verbose "Loaded $(($Global:CachedSystems.Keys).Count) valid systems from cache"
                return $true
            } else {
                Write-Verbose "Cache validation failed - cache appears empty or corrupt"
                $Global:CachedSystems = @{} # Reset to empty
            }
        } else {
